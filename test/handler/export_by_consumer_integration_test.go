...
func TestHandleExportByConsumer_ShouldReturnErrorWithWrongParameters(t *testing.T) {
	assert := as.New(t)
	...
	h := handler.NewExportByConsumerHandler(s)
	...
	err := h.HandleExportByConsumer(context.Background())

	assert.NotNil(err)
	assert.Same(err, handler.ErrConsumerParameterNotFound)
	...
	assert.NotNil(err)
	assert.Same(err, handler.ErrConsumerParameterCouldNotBeEmpty)
}

func TestHandleExportByConsumer_ShouldExportLogs(t *testing.T) {
	assert := as.New(t)

	var logs []*apigateway.Log

	serviceID := "29a5a16b-e4fa-331f-9f1c-5adea563d7de"
	consumerID := "c3e86413-648a-3552-90c3-b13491ee07d6"
	userIP := "0.0.0.0"
	...
	driverMock := mock.DriverMock{}
	driverMock.On("GetByConsumer", consumerID, itemsPerPage).Return(logs).Twice()
...
func TestHandleExportByConsumer_ShouldPaginateLogs(t *testing.T) {
	assert := as.New(t)
	...
	driverMock := mock.DriverMock{}
	driverMock.On("GetByConsumer", consumerID, itemsPerPage).Return(logs).Times(3)
...
func TestHandleExportByConsumer_ShouldReturnErrorOnGettingLogs(t *testing.T) {
	assert := as.New(t)
	...
	driverMock := mock.DriverMock{}
	driverMock.On("GetByConsumer", consumerID, itemsPerPage).Return(nil, driverErr).Twice()
...
func TestHandleExportByConsumer_ShouldReturnErrorOnWritingLogs(t *testing.T) {
	assert := as.New(t)
	...
	driverMock := mock.DriverMock{}
	driverMock.On("GetByConsumer", consumerID, itemsPerPage).Return(logs, nil).Twice()
